from flask import Flask, render_template, request, jsonify, send_file
import sys
import os
import json
from datetime import datetime
import uuid

# Add backend to path
sys.path.append(os.path.dirname(os.path.abspath(__file__)))

# Import backend modules with error handling
try:
    from backend.scanner import SQLiScanner
except ImportError as e:
    print(f"Warning: Error importing scanner: {e}")
    print("Using mock scanner for testing")
    
    # Mock scanner for testing
    class SQLiScanner:
        def __init__(self, target_url, deep_scan=False):
            self.target_url = target_url
            self.deep_scan = deep_scan
            self.scan_id = str(uuid.uuid4())[:8]
        
        def start_scan(self):
            # Return mock vulnerabilities
            return [
                {
                    'parameter': 'id',
                    'payload': "' OR '1'='1",
                    'finding': {'type': 'Error-based SQL Injection', 'evidence': 'MySQL syntax error detected in response'},
                    'location': self.target_url,
                    'timestamp': datetime.now().isoformat()
                },
                {
                    'parameter': 'page',
                    'payload': "' AND SLEEP(5)",
                    'finding': {'type': 'Time-based Blind SQL Injection', 'evidence': '5 second delay observed'},
                    'location': self.target_url,
                    'timestamp': datetime.now().isoformat()
                }
            ]

app = Flask(__name__)
app.config['SECRET_KEY'] = 'your-secret-key-change-this'

# Ensure directories exist
os.makedirs('reports', exist_ok=True)
os.makedirs('logs', exist_ok=True)

@app.route('/')
def index():
    """Home page"""
    try:
        return render_template('index.html')
    except Exception as e:
        return f"Error loading template: {e}", 500

@app.route('/api/scan', methods=['POST'])
def scan():
    """Start a new scan"""
    try:
        data = request.get_json()
        if not data:
            return jsonify({'error': 'No JSON data received'}), 400
        
        target_url = data.get('url')
        deep_scan = data.get('deep_scan', False)
        
        if not target_url:
            return jsonify({'error': 'Please provide a URL'}), 400
        
        # Validate URL format
        if not target_url.startswith(('http://', 'https://')):
            target_url = 'https://' + target_url
        
        print(f"[*] Starting scan on: {target_url}")
        print(f"[*] Deep scan: {deep_scan}")
        
        # Initialize scanner
        scanner = SQLiScanner(target_url, deep_scan)
        
        # Run scan
        vulnerabilities = scanner.start_scan()
        
        # Calculate risk score
        risk_score = 0
        for vuln in vulnerabilities:
            finding_type = vuln.get('finding', {}).get('type', '')
            if 'Error' in finding_type:
                risk_score += 40
            elif 'Time' in finding_type:
                risk_score += 30
            elif 'Boolean' in finding_type:
                risk_score += 20
            else:
                risk_score += 15
        risk_score = min(risk_score, 100)
        
        # Prepare response
        response = {
            'success': True,
            'target': target_url,
            'deep_scan': deep_scan,
            'total_vulnerabilities': len(vulnerabilities),
            'risk_score': risk_score,
            'vulnerabilities': [],
            'scan_id': scanner.scan_id,
            'timestamp': datetime.now().isoformat()
        }
        
        # Format vulnerabilities for JSON response
        for vuln in vulnerabilities:
            finding_type = vuln.get('finding', {}).get('type', 'Unknown')
            severity = 'CRITICAL' if 'Error' in finding_type else 'HIGH' if 'Time' in finding_type else 'MEDIUM'
            response['vulnerabilities'].append({
                'parameter': vuln.get('parameter', 'Unknown'),
                'payload': vuln.get('payload', 'Unknown'),
                'type': finding_type,
                'evidence': vuln.get('finding', {}).get('evidence', 'No evidence'),
                'severity': severity,
                'location': vuln.get('location', 'Unknown'),
                'timestamp': vuln.get('timestamp', datetime.now().isoformat())
            })
        
        # Save report to file
        report_filename = f"reports/scan_{scanner.scan_id}.json"
        with open(report_filename, 'w') as f:
            json.dump(response, f, indent=2)
        
        print(f"[✓] Scan completed. Found {len(vulnerabilities)} vulnerabilities")
        print(f"[✓] Report saved: {report_filename}")
        
        return jsonify(response)
        
    except Exception as e:
        import traceback
        traceback.print_exc()
        return jsonify({'error': str(e), 'traceback': traceback.format_exc()}), 500

@app.route('/api/reports', methods=['GET'])
def get_reports():
    """Get list of all scan reports"""
    try:
        reports = []
        reports_dir = 'reports'
        
        if os.path.exists(reports_dir):
            for filename in os.listdir(reports_dir):
                if filename.endswith('.json'):
                    filepath = os.path.join(reports_dir, filename)
                    try:
                        with open(filepath, 'r') as f:
                            report_data = json.load(f)
                            reports.append({
                                'filename': filename,
                                'scan_id': report_data.get('scan_id'),
                                'target': report_data.get('target', 'Unknown'),
                                'timestamp': report_data.get('timestamp', datetime.now().isoformat()),
                                'vulnerabilities': report_data.get('total_vulnerabilities', 0),
                                'risk_score': report_data.get('risk_score', 0)
                            })
                    except Exception as e:
                        print(f"Error reading {filename}: {e}")
                        continue
        
        # Sort by timestamp descending (newest first)
        reports.sort(key=lambda x: x.get('timestamp', ''), reverse=True)
        return jsonify({'reports': reports})
    except Exception as e:
        return jsonify({'error': str(e)}), 500

@app.route('/api/reports/<filename>', methods=['GET'])
def get_report(filename):
    """Get specific scan report"""
    try:
        # Security: prevent path traversal
        filename = os.path.basename(filename)
        filepath = os.path.join('reports', filename)
        
        if os.path.exists(filepath):
            with open(filepath, 'r') as f:
                report_data = json.load(f)
            return jsonify(report_data)
        else:
            return jsonify({'error': 'Report not found'}), 404
    except Exception as e:
        return jsonify({'error': str(e)}), 500

@app.route('/api/export/<filename>', methods=['GET'])
def export_report(filename):
    """Download report as JSON file"""
    try:
        # Security: prevent path traversal
        filename = os.path.basename(filename)
        filepath = os.path.join('reports', filename)
        
        if os.path.exists(filepath):
            return send_file(filepath, as_attachment=True, download_name=filename)
        else:
            return jsonify({'error': 'Report not found'}), 404
    except Exception as e:
        return jsonify({'error': str(e)}), 500

if __name__ == '__main__':
    print("""
    ╔═══════════════════════════════════════════════════════════════════════╗
    ║     SQL INJECTION DETECTOR - ADVANCED SECURITY DASHBOARD              ║
    ║                                                                       ║
    ║     Server starting at: http://localhost:5000                         ║
    ║                                                                       ║
    ║     Features:                                                         ║
    ║     • Real-time vulnerability detection                               ║
    ║     • Risk scoring and impact analysis                                ║
    ║     • JSON report generation                                          ║
    ║     • Rotating background lighting                                    ║
    ║     • Animated UI with modern design                                  ║
    ║                                                                       ║
    ║     ⚠️  IMPORTANT: Only use on authorized targets!                    ║
    ╚═══════════════════════════════════════════════════════════════════════╝
    """)
    app.run(debug=True, host='127.0.0.1', port=5000, use_reloader=False)